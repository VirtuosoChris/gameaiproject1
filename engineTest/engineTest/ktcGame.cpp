...
ktcGame::ktcGame(irr::IrrlichtDevice *device, irr::scene::ITriangleSelector* selector, gameHUD* display):can (device), graph (device, "NODE_LIST.txt","ADJACENCY_LIST.txt","EXCLUDE.txt"), agent2 (Model("../media/chuckie.MD2","../media/Chuckie.pcx", device), irr::core::vector3df(0,0,0), 300000, 10000, PREY, core::vector3df(-528.744751, 0.024357, 102.937782), device->getSceneManager(), &graph), plyr(device, irr::core::vector3df(0,0,0), 300000, 0, PREY){
	graph.selector = selector; 	graph.toggleDebugOutput(false);	dMode = NONE;
	...
	agent2.getSceneNode()->setPosition(spawnPointList[1]);	agent2.setPosition(spawnPointList[1]);
	...
	//camera = 	//smgr->addCameraSceneNodeFPS();// addCameraSceneNodeFPS();
	//CPTODO: REPLACE HARD CODED CONSTANT WITH SOMETHING BETTER	//camera->setPosition(core::vector3df(-280,288,-830));	plyr.setSpawnPoint(spawnPointList[0]);	plyr.getSceneNode()->setPosition( spawnPointList[0] );	//agent2.createPatrolRoute(&graph);
...
void ktcGame::RoundRobin(std::vector<GamePlayer *> plst){	for(int i = 0; i < plst.size(); i++){		//if i'm a predator, set me to prey		if( ( (*plst[i]).getPlayerType() == PREDATOR) && (i+1 != plst.size()) ){			(*plst[i]).setPlayerType(PREY);			(*plst[i]).setSpeed();			//change state to init of pred and prey			(*plst[i+1]).setPlayerType(PREDATOR);			(*plst[i+1]).setSpeed();			break;		}
		...
		//if i'm a prey, set me to predator		if( ( (*plst[i]).getPlayerType() == PREY) && (i+1 != plst.size()) ){			(*plst[i]).setPlayerType(PREDATOR);			(*plst[i]).setSpeed();			//change state to init of pred and prey			(*plst[i+1]).setPlayerType(PREY);			(*plst[i+1]).setSpeed();			break;		}
		//if i'm a prey at the end of the list, do special indexing shit		else if( ((*plst[i]).getPlayerType() == PREY) && ( (i+1) == plst.size() ) ){			//change state to init of pred and prey			(*plst[i]).setPlayerType(PREDATOR);			(*plst[i]).setSpeed();			(*plst[i% (plst.size() - 1) ]).setPlayerType(PREY);			(*plst[i% (plst.size() - 1) ]).setSpeed();			break;		}	}
}
